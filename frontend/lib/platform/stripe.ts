...
export function professionalStripePriceId() {
  ...
  if (!value) {
    throw new Error(
      "STRIPE_PRICE_PROFESSIONAL_MONTHLY is not configured.",
    );
  }
  return value;
}
...
export async function stripeRequest<
  T extends StripeObject = StripeObject,
>(
  path: string,
  ...
  if (!response.ok) {
    throw new Error(
      result.error?.message || "Stripe request failed.",
    );
  }
...
export function verifyStripeWebhook(
  ...
  const ageSeconds = Math.abs(
    Date.now() / 1000 - numericTimestamp,
  );
